...
const AdminCalendar: React.FC<AdminCalendarProps> = ({ darkMode = false }) => {
  ...
  const [statusFilter, setStatusFilter] = useState<'pending' | 'editing' | 'completed' | null>(null);
  ...
  return (
    <div className={`flex h-full w-full transition-colors ${darkMode ? 'bg-black' : 'bg-white'}`}>
      ...
      {selectedEvent && (
        <div className={`fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 transition-colors ${darkMode ? 'bg-black/70' : 'bg-white/70'}`} onClick={() => {
